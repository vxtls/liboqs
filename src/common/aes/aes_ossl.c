...
static void AES128_ECB_load_schedule(const uint8_t *key, void **schedule) {
	*schedule = malloc(sizeof(struct key_schedule));
	...
	ks->for_ECB = 1;
	ks->ctx = OSSL_FUNCALL(EVP_CIPHER_CTX_new());
	OQS_EXIT_IF_NULLPTR(ks->ctx, "OpenSSL");
	OQS_OPENSSL_GUARD(OSSL_FUNCALL(EVP_EncryptInit_ex(ks->ctx, oqs_aes_128_ecb(), NULL, key, NULL)));
	OSSL_FUNCALL(EVP_CIPHER_CTX_set_padding(ks->ctx, 0));
}
...
static void AES128_free_schedule(void *schedule) {
	if (schedule != NULL) {
		struct key_schedule *ks = (struct key_schedule *) schedule;
		if (ks->ctx != NULL) {
			OSSL_FUNCALL(EVP_CIPHER_CTX_free(ks->ctx));
		}
...
static void AES128_ECB_enc_sch(const uint8_t *plaintext, const size_t plaintext_len, const void *schedule, uint8_t *ciphertext) {
	...
	OQS_OPENSSL_GUARD(OSSL_FUNCALL(EVP_EncryptFinal_ex(ks->ctx, ciphertext, &outlen)));
}
...
static void AES256_ECB_load_schedule(const uint8_t *key, void **schedule) {
	*schedule = malloc(sizeof(struct key_schedule));
	...
	ks->ctx = OSSL_FUNCALL(EVP_CIPHER_CTX_new());
	OQS_EXIT_IF_NULLPTR(ks->ctx, "OpenSSL");
	OQS_OPENSSL_GUARD(OSSL_FUNCALL(EVP_EncryptInit_ex(ks->ctx, oqs_aes_256_ecb(), NULL, key, NULL)));
	OSSL_FUNCALL(EVP_CIPHER_CTX_set_padding(ks->ctx, 0));
}
...
static void AES256_CTR_inc_init(const uint8_t *key, void **schedule) {
	*schedule = malloc(sizeof(struct key_schedule));
	struct key_schedule *ks = (struct key_schedule *) *schedule;
	EVP_CIPHER_CTX *ctr_ctx = OSSL_FUNCALL(EVP_CIPHER_CTX_new());
	assert(ctr_ctx != NULL);
...
static void AES256_CTR_inc_iv(const uint8_t *iv, size_t iv_len, void *schedule) {
	...
	OQS_OPENSSL_GUARD(OSSL_FUNCALL(EVP_EncryptInit_ex(ks->ctx, oqs_aes_256_ctr(), NULL, ks->key, ks->iv)));
}
...
static void AES256_CTR_inc_stream_iv(const uint8_t *iv, size_t iv_len, const void *schedule, uint8_t *out, size_t out_len) {
	...
	const struct key_schedule *ks = (const struct key_schedule *) schedule;
	OQS_OPENSSL_GUARD(OSSL_FUNCALL(EVP_EncryptInit_ex(ctr_ctx, oqs_aes_256_ctr(), NULL, ks->key, iv_ctr)));

	SIZE_T_TO_INT_OR_EXIT(out_len, out_len_input_int)
	memset(out, 0, (size_t)out_len_input_int);
	int out_len_output;
	OQS_OPENSSL_GUARD(OSSL_FUNCALL(EVP_EncryptUpdate(ctr_ctx, out, &out_len_output, out, out_len_input_int)));
	OQS_OPENSSL_GUARD(OSSL_FUNCALL(EVP_EncryptFinal_ex(ctr_ctx, out + out_len_output, &out_len_output)));
	OSSL_FUNCALL(EVP_CIPHER_CTX_free(ctr_ctx));
}
...
static void AES256_CTR_inc_stream_blks(void *schedule, uint8_t *out, size_t out_blks) {
	...
	SIZE_T_TO_INT_OR_EXIT(out_len, out_len_input_int);
	memset(out, 0, (size_t)out_len_input_int);
	OQS_OPENSSL_GUARD(OSSL_FUNCALL(EVP_EncryptUpdate(ks->ctx, out, &out_len_output, out, (int) out_len)));
}
