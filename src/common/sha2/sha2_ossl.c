...
static void do_hash(uint8_t *output, const uint8_t *input, size_t inplen, const EVP_MD *md) {
	...
	OSSL_FUNCALL(EVP_DigestFinal_ex(mdctx, output, &outlen));
...
static void SHA2_sha256_inc_init(OQS_SHA2_sha256_ctx *state) {
	...
	mdctx = OSSL_FUNCALL(EVP_MD_CTX_new());
	OQS_EXIT_IF_NULLPTR(mdctx, "OpenSSL");
	OQS_OPENSSL_GUARD(OSSL_FUNCALL(EVP_DigestInit_ex(mdctx, md, NULL)));
	state->ctx = mdctx;
}

static void SHA2_sha256_inc_blocks(OQS_SHA2_sha256_ctx *state, const uint8_t *in, size_t inblocks) {
	OQS_OPENSSL_GUARD(OSSL_FUNCALL(EVP_DigestUpdate((EVP_MD_CTX *) state->ctx, in, inblocks * SHA2_BLOCK_SIZE)));
}

static void SHA2_sha256_inc_finalize(uint8_t *out, OQS_SHA2_sha256_ctx *state, const uint8_t *in, size_t inlen) {
	unsigned int md_len;
	if (inlen > 0) {
		OQS_OPENSSL_GUARD(OSSL_FUNCALL(EVP_DigestUpdate((EVP_MD_CTX *) state->ctx, in, inlen)));
	}
	OQS_OPENSSL_GUARD(OSSL_FUNCALL(EVP_DigestFinal_ex((EVP_MD_CTX *) state->ctx, out, &md_len)));
...
static void SHA2_sha256_inc_ctx_clone(OQS_SHA2_sha256_ctx *dest, const OQS_SHA2_sha256_ctx *src) {
	...
	OQS_OPENSSL_GUARD(OSSL_FUNCALL(EVP_MD_CTX_copy_ex((EVP_MD_CTX *) dest->ctx, (EVP_MD_CTX *) src->ctx)));
}
...
static void SHA2_sha512_inc_blocks(OQS_SHA2_sha512_ctx *state, const uint8_t *in, size_t inblocks) {
	OQS_OPENSSL_GUARD(OSSL_FUNCALL(EVP_DigestUpdate((EVP_MD_CTX *) state->ctx, in, inblocks * 2 * SHA2_BLOCK_SIZE)));
}
