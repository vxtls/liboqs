...
static void handleErrors(void) {
	OSSL_FUNCALL(ERR_print_errors_fp(stderr));
	abort();
}
#endif
...
static void AES256_ECB(unsigned char *key, unsigned char *ctr, unsigned char *buffer) {
	...
	if (!(ctx = OSSL_FUNCALL(EVP_CIPHER_CTX_new()))) {
		handleErrors();
	}

	if (1 != OSSL_FUNCALL(EVP_EncryptInit_ex(ctx, oqs_aes_256_ecb(), NULL, key, NULL))) {
		handleErrors();
	}

	if (1 != OSSL_FUNCALL(EVP_EncryptUpdate(ctx, buffer, &len, ctr, 16))) {
		handleErrors();
	}

	/* Clean up */
	OSSL_FUNCALL(EVP_CIPHER_CTX_free(ctx));
#else
	void *schedule = NULL;
