...
static void SHA3_shake128_x4_inc_init(OQS_SHA3_shake128_x4_inc_ctx *state) {
	...
	s->mdctx0 = OSSL_FUNCALL(EVP_MD_CTX_new());
	s->mdctx1 = OSSL_FUNCALL(EVP_MD_CTX_new());
	s->mdctx2 = OSSL_FUNCALL(EVP_MD_CTX_new());
	s->mdctx3 = OSSL_FUNCALL(EVP_MD_CTX_new());
	OSSL_FUNCALL(EVP_DigestInit_ex(s->mdctx0, oqs_shake128(), NULL));
	OSSL_FUNCALL(EVP_DigestInit_ex(s->mdctx1, oqs_shake128(), NULL));
	OSSL_FUNCALL(EVP_DigestInit_ex(s->mdctx2, oqs_shake128(), NULL));
	OSSL_FUNCALL(EVP_DigestInit_ex(s->mdctx3, oqs_shake128(), NULL));
	s->n_out = 0;
}
...
static void SHA3_shake128_x4_inc_absorb(OQS_SHA3_shake128_x4_inc_ctx *state, const uint8_t *in0, const uint8_t *in1, const uint8_t *in2, const uint8_t *in3, size_t inplen) {
	intrn_shake128_x4_inc_ctx *s = (intrn_shake128_x4_inc_ctx *)state->ctx;
	OSSL_FUNCALL(EVP_DigestUpdate(s->mdctx0, in0, inplen));
	OSSL_FUNCALL(EVP_DigestUpdate(s->mdctx1, in1, inplen));
	OSSL_FUNCALL(EVP_DigestUpdate(s->mdctx2, in2, inplen));
	OSSL_FUNCALL(EVP_DigestUpdate(s->mdctx3, in3, inplen));
}
...
static void SHA3_shake128_x4_inc_squeeze(uint8_t *out0, uint8_t *out1, uint8_t *out2, uint8_t *out3, size_t outlen, OQS_SHA3_shake128_x4_inc_ctx *state) {
	...
	clone = OSSL_FUNCALL(EVP_MD_CTX_new());
	OSSL_FUNCALL(EVP_DigestInit_ex(clone, oqs_shake128(), NULL));
	if (s->n_out == 0) {
		OSSL_FUNCALL(EVP_MD_CTX_copy_ex(clone, s->mdctx0));
		OSSL_FUNCALL(EVP_DigestFinalXOF(clone, out0, outlen));
		OSSL_FUNCALL(EVP_MD_CTX_copy_ex(clone, s->mdctx1));
		OSSL_FUNCALL(EVP_DigestFinalXOF(clone, out1, outlen));
		OSSL_FUNCALL(EVP_MD_CTX_copy_ex(clone, s->mdctx2));
		OSSL_FUNCALL(EVP_DigestFinalXOF(clone, out2, outlen));
		OSSL_FUNCALL(EVP_MD_CTX_copy_ex(clone, s->mdctx3));
		OSSL_FUNCALL(EVP_DigestFinalXOF(clone, out3, outlen));
	} else {
		uint8_t *tmp;
		...
		OSSL_FUNCALL(EVP_MD_CTX_copy_ex(clone, s->mdctx0));
		OSSL_FUNCALL(EVP_DigestFinalXOF(clone, tmp, s->n_out + outlen));
	...
	OSSL_FUNCALL(EVP_MD_CTX_free(clone));
	s->n_out += outlen;
}
...
static void SHA3_shake128_x4_inc_ctx_clone(OQS_SHA3_shake128_x4_inc_ctx *dest, const OQS_SHA3_shake128_x4_inc_ctx *src) {
	...
	OSSL_FUNCALL(EVP_MD_CTX_copy_ex(d->mdctx0, s->mdctx0));
	OSSL_FUNCALL(EVP_MD_CTX_copy_ex(d->mdctx1, s->mdctx1));
	OSSL_FUNCALL(EVP_MD_CTX_copy_ex(d->mdctx2, s->mdctx2));
	OSSL_FUNCALL(EVP_MD_CTX_copy_ex(d->mdctx3, s->mdctx3));
	d->n_out = s->n_out;
}

static void SHA3_shake128_x4_inc_ctx_release(OQS_SHA3_shake128_x4_inc_ctx *state) {
	intrn_shake128_x4_inc_ctx *s = (intrn_shake128_x4_inc_ctx *)state->ctx;
	OSSL_FUNCALL(EVP_MD_CTX_free(s->mdctx0));
	OSSL_FUNCALL(EVP_MD_CTX_free(s->mdctx1));
	OSSL_FUNCALL(EVP_MD_CTX_free(s->mdctx2));
	OSSL_FUNCALL(EVP_MD_CTX_free(s->mdctx3));
	free(s); // IGNORE free-check
}

static void SHA3_shake128_x4_inc_ctx_reset(OQS_SHA3_shake128_x4_inc_ctx *state) {
	intrn_shake128_x4_inc_ctx *s = (intrn_shake128_x4_inc_ctx *)state->ctx;
	OSSL_FUNCALL(EVP_MD_CTX_reset(s->mdctx0));
	OSSL_FUNCALL(EVP_MD_CTX_reset(s->mdctx1));
	OSSL_FUNCALL(EVP_MD_CTX_reset(s->mdctx2));
	OSSL_FUNCALL(EVP_MD_CTX_reset(s->mdctx3));
	OSSL_FUNCALL(EVP_DigestInit_ex(s->mdctx0, oqs_shake128(), NULL));
	OSSL_FUNCALL(EVP_DigestInit_ex(s->mdctx1, oqs_shake128(), NULL));
	OSSL_FUNCALL(EVP_DigestInit_ex(s->mdctx2, oqs_shake128(), NULL));
	OSSL_FUNCALL(EVP_DigestInit_ex(s->mdctx3, oqs_shake128(), NULL));
...
static void SHA3_shake256_x4_inc_init(OQS_SHA3_shake256_x4_inc_ctx *state) {
	...
	s->mdctx2 = OSSL_FUNCALL(EVP_MD_CTX_new());
	s->mdctx3 = OSSL_FUNCALL(EVP_MD_CTX_new());
	OSSL_FUNCALL(EVP_DigestInit_ex(s->mdctx0, oqs_shake256(), NULL));
	OSSL_FUNCALL(EVP_DigestInit_ex(s->mdctx1, oqs_shake256(), NULL));
	OSSL_FUNCALL(EVP_DigestInit_ex(s->mdctx2, oqs_shake256(), NULL));
	OSSL_FUNCALL(EVP_DigestInit_ex(s->mdctx3, oqs_shake256(), NULL));
	s->n_out = 0;
}
...
static void SHA3_shake256_x4_inc_squeeze(uint8_t *out0, uint8_t *out1, uint8_t *out2, uint8_t *out3, size_t outlen, OQS_SHA3_shake256_x4_inc_ctx *state) {
	...
	clone = OSSL_FUNCALL(EVP_MD_CTX_new());
	OSSL_FUNCALL(EVP_DigestInit_ex(clone, oqs_shake256(), NULL));
	if (s->n_out == 0) {
		OSSL_FUNCALL(EVP_MD_CTX_copy_ex(clone, s->mdctx0));
		OSSL_FUNCALL(EVP_DigestFinalXOF(clone, out0, outlen));
		OSSL_FUNCALL(EVP_MD_CTX_copy_ex(clone, s->mdctx1));
